...
test("resource manager aggregate stack covers all Terraform deployment modules", () => {
  ...
  assert.match(terraform, /count = var\.enabled && !var\.deploy_only_app \? 1 : 0/);
  ...
  assert.match(terraform, /id = var\.deploy_only_app \? oci_devops_build_pipeline_stage\.deploy_portal\[0\]\.id : \(!contains\(keys\(oci_devops_build_pipeline_stage\.deliver_image\), each\.key\) \? oci_devops_build_pipeline\.this\[0\]\.id : oci_devops_build_pipeline_stage\.deliver_image\[each\.key\]\.id\)/);
  ...
  assert.match(terraform, /id\s+=\s+var\.deploy_only_app \? oci_devops_build_pipeline_stage\.deploy_portal\[0\]\.id/);
