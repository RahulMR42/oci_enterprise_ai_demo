resource "terraform_data" "langgraph_hosted_agentic_application" {
  ...
  input = {
    app_source_dir                  = abspath("${path.module}/${var.langgraph_app_source_dir}")
    ...
    profile                         = var.profile
    provisioning_revision           = "20260513-add-langgraph-hosted-application"
...
PY
      ...
      application_file='${self.input.generated_dir}/langgraph_hosted_application.json'
      deployment_file='${self.input.generated_dir}/langgraph_hosted_deployment.json'
...
PY
      ...
      oci generative-ai hosted-application create \
        ...
        --description 'Enterprise AI demo LangGraph MCP hosted agent application.' \
...
PY
      ...
      if [ -z "$hosted_application_id" ]; then
        echo "OCI LangGraph hosted application create response did not include an id." >&2
        exit 1
      fi
...
def load_json(path):
    ...
    try:
        ...
        if start >= 0 and end > start:
            return json.loads(text[start:end + 1])
        raise
repository = load_json(generated / 'langgraph_ocir_repository.json').get('data', {})
application = load_json(generated / 'langgraph_hosted_application.json').get('data', {})
deployment = load_json(generated / 'langgraph_hosted_deployment.json').get('data', {})
...
runtime = {
    ...
    'imageUri': '$image_uri',
    'runtime': 'langgraph',
    'mcpServer': 'hosted-langgraph-mcp-gateway',
...
(generated / 'langgraph_hosted_agent.json').write_text(json.dumps(runtime, indent=2))
...
PY
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      set -euo pipefail
      runtime_file='${self.input.generated_dir}/langgraph_hosted_agent.json'
      hosted_application_file='${self.input.generated_dir}/langgraph_hosted_application.json'
      hosted_deployment_file='${self.input.generated_dir}/langgraph_hosted_deployment.json'
      repository_file='${self.input.generated_dir}/langgraph_ocir_repository.json'
