resource "terraform_data" "hosted_agentic_application" {
  triggers_replace = [
    "20260512-refresh-hosted-deployment-after-create",
    var.resource_suffix,
    var.repository_name,
    var.image_tag,
    var.idcs_domain_url,
  ...
  input = {
    app_source_dir                  = abspath("${path.module}/${var.app_source_dir}")
    ...
    profile                         = var.profile
    provisioning_revision           = "20260512-refresh-hosted-deployment-after-create"
...
PY
      ...
      printf '%s\n' "$repository_json" > '${self.input.generated_dir}/ocir_repository.json'
      ...
      if [ '${self.input.push_image}' = 'true' ]; then
        ...
        echo "Skipping docker build/push because push_image=false. Expected image URI: $image_uri"
      fi

      application_file='${self.input.generated_dir}/hosted_application.json'
      deployment_file='${self.input.generated_dir}/hosted_deployment.json'
...
PY
      ...
      oci generative-ai hosted-application create \
        ...
        --description 'Enterprise AI demo hosted agent application.' \
        ...
        --freeform-tags '{"enterprise-ai-demo":"true","demo":"hosted-agentic-applications"}' \
        $oci_auth_args \
        --region '${self.input.region}' \
...
def load_json(path):
    ...
    try:
        ...
        if start >= 0 and end > start:
            return json.loads(text[start:end + 1])
        raise
repository = load_json(generated / 'ocir_repository.json').get('data', {})
application = load_json(generated / 'hosted_application.json').get('data', {})
deployment = load_json(generated / 'hosted_deployment.json').get('data', {})
...
(generated / 'hosted_agent.json').write_text(json.dumps(runtime, indent=2))
...
PY
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      set -euo pipefail
      runtime_file='${self.input.generated_dir}/hosted_agent.json'
      hosted_application_file='${self.input.generated_dir}/hosted_application.json'
      hosted_deployment_file='${self.input.generated_dir}/hosted_deployment.json'
      repository_file='${self.input.generated_dir}/ocir_repository.json'
...
PY
      ...
      rm -f '${self.input.generated_dir}/hosted_agent.json' '${self.input.generated_dir}/hosted_deployment.json' '${self.input.generated_dir}/hosted_application.json' '${self.input.generated_dir}/ocir_repository.json'
    EOT
  }

  depends_on = [oci_artifacts_container_repository.hosted_agent]
}
